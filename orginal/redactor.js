...
Redactor.add('module', 'editor', {
    ...
    start() {
        ...
        this.inputComposition = this.dom('<input type="text" id="rxCompositionCutter' + this.uuid + '" style="position: fixed; z-index: -1000; width: 1px; height: 1px;" />');
...
Redactor.add('module', 'format', {
    ...
    _replaceListTodoToText(mode, type, instance) {
        ...
        $items.each($node => {
            ...
            let itemBlock = (mode === 'text') ? this.app.create('block.text', { content: itemInstance.getContent(true) }) : this.app.create('block.heading', { level: this.tag.replace('h', ''), content: itemInstance.getContent(true) });
