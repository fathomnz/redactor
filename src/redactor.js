...
(function () {
  ...
  Dom.prototype = {
    ...
    _getHeightOrWidth(name, type) {
      ...
      var $targets = this.parents().filter(function ($n) {
        var node = $n.get();
        return node.nodeType === 1 && getComputedStyle(node, null).display === 'none'
          ? node
          : false;
      });
  ...
  // Dom & Ajax
  Redactor.dom = function (selector, context) {
  ...
  Redactor.opts = {
    ...
    quote: {
      template:
        '<blockquote><p data-placeholder="Quote..."></p><p><cite data-placeholder="Attribution"></cite></p></blockquote>',
    },
    embed: {
    ...
    regex: {
      ...
      youtube:
        /^https?\:\/\/(?:www\.youtube(?:\-nocookie)?\.com\/|m\.youtube\.com\/|youtube\.com\/)?(?:ytscreeningroom\?vi?=|youtu\.be\/|vi?\/|live|user\/.+\/u\/\w{1,2}\/|embed\/|watch\?(?:.*\&)?vi?=|\&vi?=|\?(?:.*\&)?vi?=)([^#\&\?\n\/<>"']*)/gi,
      vimeo:
  ...
  Redactor.lang['en'] = {
    ...
    embed: {
      ...
      poster: 'Poster image',
      caption: 'Caption',
      description: 'Paste any video share link or enter the url (vimeo or youtube video only)',
  ...
  Redactor.add('mixin', 'block', {
    ...
    getNextParent() {
      let parent = this.isType('todoitem')
        ? this.getClosest(['todo'])
        : this.getClosest(['table', 'wrapper', 'layout']);
      let next = false;
      if (parent) {
        next = parent.getNext();
      }
    ...
    getNext(type) {
      ...
      return $el.length !== 0 && $el.is(this._buildTraverseSelector(type))
        ? $el.dataget('instance')
        : false;
    },
    getPrev(type) {
    ...
    getClassname(items) {
      ...
      if (items) {
        ...
        $clone.removeClass(
          'rx-block-placeholder rx-block-focus rx-layout-grid rx-block-control-focus data-rx-parsed data-rx-first-level data-rx-inline data-rx-focusable'
        );
    ...
    unparseInlineBlocks($el) {
      $el
        .find('[data-rx-inline]')
        .removeAttr('data-rx-type data-rx-inline contenteditable tabindex')
        .removeClass('rx-block-focus');
      if ($el.attr('class') === '') {
        $el.removeAttr('class');
      }
    ...
    _renderInlineBlocks($el, force) {
      ...
      $el = $el || this.$block;
      $el.find('[' + this.opts.get('dataBlock') + ']').each(
        function ($node) {
  ...
  Redactor.add('mixin', 'tool', {
    ...
    _buildLabel() {
      if (this.type !== 'checkbox' && this._has('label')) {
        ...
        if (this.obj.hint) {
          let $hint = this.dom('<span class="rx-form-hint">').html(
            '(' + this.lang.parse(this.obj.hint) + ')'
          );
  ...
  Redactor.add('class', 'opts', {
    ...
    set(name, value) {
      name
        .split('.')
        .reduce(
          (o, p, i, parts) => (o[p] = i === parts.length - 1 ? value : o[p] || {}),
          this.opts
        );
    },
    get(name) {
  ...
  Redactor.add('class', 'lang', {
    ...
    get(name) {
      let value =
        this._fetchFromVars(name) || (this.langKey !== 'en' && this._fetchFromDefaultLang(name));
  ...
  Redactor.add('class', 'marker', {
    ...
    _replaceTextWithMarkers(html) {
      ...
      return html.replace('---marker-start---', startMarker).replace('---marker-end---', endMarker);
    },
    _remove(el) {
      if (el) {
    ...
    _buildMarker(pos = 'start') {
      ...
      $marker
        .attr('id', `rx-selection-marker-${pos}`)
    ...
    _getNextNode(marker) {
      return marker.nextSibling &&
        marker.nextSibling.nodeType === 3 &&
        marker.nextSibling.textContent.replace(/[\n\t]/g, '') === ''
        ? false
        : marker.nextSibling;
    },
  ...
  Redactor.add('class', 'caret', {
    ...
    _setEnd(range, node) {
      ...
      if (lastInline) {
        if (
          ...
          last.childNodes[0].nodeType === 1 &&
          ['svg', 'br'].includes(last.childNodes[0].tagName.toLowerCase())
        ) {
    ...
    _getPosition(node, trimmed, br) {
      ...
      let str =
        isTextNode || trimmed === false ? contentHolder.innerHTML : contentHolder.innerHTML.trim();
  ...
  Redactor.add('class', 'selection', {
    ...
    isCursorInFirstOrLastLine() {
      ...
      if (!selection.rangeCount) return { isFirstLine: false, isLastLine: false };

      const range = selection.getRangeAt(0);

      // Create a new range from the start to the current cursor position
      const rangeToCursor = this.app.getDocNode().createRange();
      ...
      const rangeFromCursor = this.app.getDocNode().createRange();
      rangeFromCursor.setStart(range.endContainer, range.endOffset);
      rangeFromCursor.setEnd(
        range.endContainer,
    ...
    getNodes(data) {
      ...
      let nodes = this.app.editor.isSelectAll()
        ? [...this.app.editor.getLayout().get().getElementsByTagName('*')]
        : this._nodes(range, data?.partial);
    ...
    getPosition(type) {
      ...
      if (this.app.getWinNode().getSelection && range.getBoundingClientRect) {
        if (
          ...
          this.isFullySelected(range.startContainer)
        ) {
    ...
    _textSel(node, selected) {
      ...
      return (
        ...
        new RegExp(`^${utils.escapeRegExp(text)}$`).test(selected)
      );
    },
    _textIn(range, node) {
      const treeWalker = this.app
        .getDocNode()
        .createTreeWalker(node, NodeFilter.SHOW_TEXT, (node) => NodeFilter.FILTER_ACCEPT, false);
  ...
  Redactor.add('class', 'utils', {
    isMobileDevice() {
      ...
      const isMobileUserAgent = /mobile|android|iphone|ipad|tablet|blackberry|phone/i.test(
        userAgent
      );
      ...
      this.dom(value).each(
        function ($node) {
          if ($node.tagName('form')) {
            ...
            Object.keys(serializedData).forEach(
              function (key) {
  ...
  Redactor.add('class', 'parser-html', {
    ...
    _parseLayersNodes(html) {
      ...
      return this.utils.wrap(
        ...
        function ($w) {
          ...
          $w.find('[data-rx-type=wrapper],[data-rx-type=column]').each(
            function ($node) {
    ...
    _isImageBlock($el, tag) {
      ...
      if (
        $cont.get() !== $el.get() ||
        $target.prevElement().length !== 0 ||
        (tag !== 'figure' && $target.nextElement().length !== 0)
      ) {
        return;
      }
  ...
  Redactor.add('class', 'parser-line', {
    ...
    _parseInlineBlocks(html) {
      ...
      return utils.wrap(
        html,
        function ($w) {
          $w.find('[' + this.opts.get('dataBlock') + ']').each(
            function ($node) {
              if (!$node.attr('data-rx-type')) {
  ...
  Redactor.add('class', 'unparser', {
    ...
    unparseClasses($el) {
      $el.removeClass(
        'rx-block-placeholder rx-block-focus rx-block-meta-focus rx-block-control-focus rx-layout-grid'
      );
    ...
    _unparseAllTags(html) {
      ...
      return utils.wrap(
        ...
        function ($w) {
          ...
          $w.find('*').removeAttr('contenteditable data-gramm_editor data-rx-cont-width');
    ...
    _unparseDataType(html, safe) {
      ...
      return utils.wrap(
        ...
        function ($w) {
          ...
          $elms.removeAttr(
            'tabindex data-rx-parsed data-rx-width data-rx-first-level data-rx-inline data-rx-focusable'
          );
          ...
          $w.find('figcaption')
            .removeAttr('data-rx-type data-placeholder')
            .each(cleaner.removeEmptyTag.bind(this));
        }.bind(this)
      );
    },
    _unparseByType($node) {
  ...
  Redactor.add('class', 'cleaner', {
          ...
          $elms.not(filterAttrs).each(($node) => {
            ...
            for (i = attrs.length - 1; i >= 0; i--) {
              ...
              if (
                node.tagName === 'A' &&
                (name === 'href' ||
                  (name === 'id' && this.opts.is('paste.keepIdAttr')) ||
                  name === 'target')
              )
                continue;
    ...
    _cleanGDocs(html) {
      ...
      html = html.replace(
        /<span[^>]*font-weight:\s?(bold|600|700)[^>]*>([\w\W]*?)<\/span>/gi,
        '<b>$2</b>'
      );
    ...
    removeBlockTagsInside(html, tags) {
      ...
      this.blockListTags = utils.removeFromArrayByValue(this.opts.get('tags.block').concat(), [
        'ul',
        'ol',
        'li',
      ]);
    ...
    removeStyleAttr(html, filter) {
      ...
      return utils.wrap(
        ...
        function ($w) {
          $w.find('*')
            .not('[data-rx-style-cache]' + filter)
            .removeAttr('style');
        }.bind(this)
      );
    },
    ...
    _revertFrame($node) {
      if (
        $node.get().attributes.length === 0 &&
        $node.find('figcaption').length === 0 &&
        $node.find('div').length === 0
      ) {
        $node.unwrap();
      } else {
    ...
    decodeSpecialCharsInAttributes(html) {
      return html.replace(/<([a-z][a-z0-9]*)\b([^>]*)>/gi, function (match, tag, attributes) {
        const decodedAttributes = attributes.replace(
          /(["'])(?:(?=(\\?))\2.)*?\1/g,
          function (attribute) {
    ...
    restoreTemplateSyntax(html, delims) {
      ...
      for (let [key, item] of Object.entries(delims)) {
        ...
        html = html.replace(
          ...
          utils.escapeBackslash(item[0]) + '$1' + utils.escapeBackslash(item[1])
        );
      }
    ...
    _sanitizeSrc($node) {
      ...
      if (
        src.search(/^javascript:/i) !== -1 ||
        (node.tagName !== 'IMG' && src.search(/^data:/i) !== -1)
      ) {
  ...
  Redactor.add('class', 'paragraphizer', {
    ...
    parseLayers(html) {
      ...
      return utils.wrap(
        html,
        function ($w) {
          $w.find('[data-rx-type="wrapper"]').each(
            function ($node) {
              code = this._parse($node.html());
              $node.html(code);
            }.bind(this)
          );
          $w.find('[data-rx-type="column"]').each(
            function ($node) {
              code = this._parse($node.html());
              $node.html(code);
            }.bind(this)
    ...
    _parse(html, typemarkup) {
      ...
      html =
        this._has('cleanBreakline') && !this._is('cleanBreakline')
          ? html
          : html.replace(/<p(.*?)><\/?br\s?\/?><\/p>/gi, '<p$1></p>');
    ...
    _replaceTag($node, i, stored) {
      let node = $node.get(),
        replacement = document.createTextNode(
          this.remStart + i + this.remEnd + 'xparagraphmarkerz'
        );
  ...
  Redactor.add('class', 'tooltip', {
    ...
    _shouldPreventTooltip($btn) {
      return (
        this.app.modal.isOpen() ||
        $btn.hasClass('disable') ||
        (this.app.dropdown.isOpen() && this.app.ui.getState().type === 'toolbar')
      );
    },
  ...
  Redactor.add('class', 'element', {
    is(el, type, filter = true) {
      ...
      const typeHandlers = {
        ...
        'block-data-not-inline': () =>
          this._isElement(node) &&
          node.hasAttribute('data-rx-type') &&
          !node.hasAttribute('data-rx-inline'),
        ...
        text: () =>
          (typeof node === 'string' && !/^\s*<(\w+|!)[^>]*>/.test(node)) || this._isTextNode(node),
    ...
    getBlocks(el, parserTags) {
      ...
      const finalNodes = Array.from(node.childNodes).filter(
        (node) => node.nodeType === 1 && tags.includes(node.tagName.toLowerCase())
      );
    ...
    _isInlineTag(tag, node) {
      ...
      return (
        tags.includes(tag.toLowerCase()) && (!node || !this.dom(node).hasClass('email-button'))
      );
    },
    _isElement(node) {
  ...
  Redactor.add('class', 'insertion', {
    ...
    _insertToOneEditable(current) {
      ...
      if (this.isLine) {
        ...
        if (current.isType('todoitem')) {
          ...
          nodes = this._insertFragment(
            { html: $tmpnodes.text() },
            this.p.caret ? this.p.caret : 'end'
          );
        }
    ...
    _insertInstance() {
      ...
      // caret
      this.p.caret = this.p.instance.isType(['table', 'quote', 'layout'])
        ? 'start'
        : this.p.caret
          ? this.p.caret
          : 'end';
      ...
      // table to table
      if (
        this.p.instance.isType('table') &&
        (current.isType('table') || current.getClosest('table'))
      ) {
    ...
    _parse(html) {
      ...
      if (this.p.parse) {
        $parsed = parser.parse(html, {
          type: this._getParseType(),
          nodes: true,
          paragraphize: this.p.paragraphize,
        });
      } else {
  ...
  Redactor.add('class', 'autoparse', {
    ...
    _storeAndReplaceTags(html, tags, singleTags, stored) {
      tags.forEach((tag) => {
        let reTags = singleTags.includes(tag)
          ? `<${tag}[^>]*>`
          : `<${tag}[^>]*>([\\w\\W]*?)</${tag}>`;
    ...
    _formatLinks(html) {
      ...
      if ((html.match(aurl1) || html.match(aurl2)) && !html.match(imageurl)) {
        ...
        html = html.replace(
          aurl1,
          (url) => `<a href="${url}"${target}>${this._subLinkText(url)}</a>`
        );
        html = html.replace(
          aurl2,
          (match, before, url) =>
            `${before}<a href="${protocol}://${url}"${target}>${this._subLinkText(url)}</a>`
        );
      }
      return html;
    },
  ...
  Redactor.add('class', 'colorpicker', {
    ...
    _buildEvents() {
      this.app.scroll
        .getTarget()
        .on('resize.rx-colorpicker scroll.rx-colorpicker', this._buildPosition.bind(this));
    ...
    _build(params, $target) {
      ...
      this.currentBackgroundColor = this.params.style.background
        ? this.params.style.background
        : false;

      // layers
      let $layer = this.dom(
        '<div class="rx-dropdown-layer rx-dropdown-layer-' + this.params.name + '">'
      ).attr('data-type', this.params.name);
      let $box, button;
      ...
      if (this.params.tabs) {
        ...
        for (let i = 0; i < tabs.length; i++) {
          let $tab = this.dom('<a href="#">')
            .attr('data-tab', tabs[i])
            .addClass('rx-dropdown-tab rx-dropdown-tab-' + tabs[i])
            .html(this.lang.get('colorpicker.' + tabs[i]));
          ...
          // layer
          $layer = this.dom(
            '<div class="rx-dropdown-layer rx-dropdown-layer-' + tabs[i] + '">'
          ).attr('data-type', tabs[i]);
          $layer.append($box);
          ...
          if (this.params.remove && this.params.remove[tabs[i]]) {
            ...
            button = this.app.create(
              'button',
              'remove',
              this._createRemoveButton(tabs[i], value),
              false,
              'dropdown'
            );
        ...
        if (this.params.remove) {
          button = this.app.create(
            'button',
            'remove',
            this._createRemoveButton(this.params.name, this.params.remove),
            false,
            'dropdown'
          );
          $layer.append(button.getElement());
    ...
    _createInput($layer, type) {
      ...
      let $label = this.dom('<label>')
        ...
        .html(this.lang.get('colorpicker.set-color'));
      let $box = this.dom('<div>').addClass('rx-form-flex');
      let $input = this.dom('<input type="input">').addClass(
        'rx-form-input rx-form-input-' + type + ' rx-in-tool'
      );
      let $button = this.dom('<button">')
        .attr('data-rule', type)
        .addClass('rx-form-button rx-in-tool')
        .html('&rarr;');
  ...
  Redactor.add('class', 'editor-iframe', {
    ...
    _buildStartHtml() {
      ...
      let frameLang = this.opts.is('frame.lang')
        ? ' lang="' + this.opts.get('frame.lang') + '"'
        : '';
      ...
      let code =
        ...
        meta +
        headScripts +
        '</head><body style="padding: 0; margin: 0; width: 100%;">' +
        layout +
        scripts +
        '</body></html>';
  ...
  Redactor.add('class', 'event', {
    ...
    _buildTargetEvents($target, events, type) {
      for (var i = 0; i < events.length; i++) {
        $target.on(
          events[i] + '.' + this.event.eventname,
          this['on' + type + events[i]].bind(this)
        );
      }
    },
  });
  ...
  class ButtonBehavior {
    ...
    fetchUpdatedObject() {
      let obj = this.app.api(
        this.props.get('observer'),
        this.props.dump(),
        this.button.name,
        this.button.type
      );
  ...
  class ButtonBuilder {
    ...
    configureTooltip() {
      if ((this.props.has('tooltip') && !this.props.get('tooltip')) || this.props.has('text'))
        return;
  ...
  Redactor.add('module', 'container', {
    ...
    _focusExternal(name) {
      ...
      let $current = this.app.$body.find(
        '.rx-' + name + '-external.rx-' + name + '-container-' + this.uuid
      );
      $current.css('display', '');
    },
    _createMain() {
      let $container = this.dom('<div>')
    ...
    _createContainer(name, target) {
      // local
      let externalTarget =
        name === 'toolbox' ? this.opts.get('toolbar.target') : this.opts.get(name + '.target');
      ...
      // build
      let $container = this.dom('<div>').addClass(
        'rx-' + name + '-container rx-' + name + '-container-' + this.uuid
      );
  ...
  Redactor.add('module', 'editor', {
    ...
    start() {
      ...
      this.inputComposition = this.dom(
        '<input type="text" id="rxCompositionCutter' +
          this.uuid +
          '" style="position: fixed; z-index: -1000; width: 1px; height: 1px;" />'
      );
      if (this.app.isMode('default')) {
        this.app.element.after(this.inputComposition);
      } else {
        this.app.getFrameBody().append(this.inputComposition);
      }
    },
    stop() {
      if (this.$overlay) {
    ...
    destroy() {
      if (!this.app.element.isTextarea()) {
        this.$editor.removeClass(
          'rx-editor rx-editor-' +
            this.uuid +
            ' rx-empty rx-editor-breakline rx-editor-disabled rx-placeholder'
        );
    ...
    setFocus(position) {
      if (position) {
        ...
        let $target =
          position === 'start'
            ? this.app.blocks.get({ first: true })
            : this.app.blocks.get({ last: true });
  ...
  Redactor.add('module', 'addbar', {
    ...
    getItems() {
      ...
      let stack = this.app.ui.loadButtons(
        buttons,
        customButtons,
        'addbar',
        false,
    ...
    popup(e, button) {
      ...
      this.app.dropdown.create('addbar', {
        items: buttons,
        extend: this.customButtons,
        remove: removeButtons,
        type: 'addbar',
      });
  ...
  Redactor.add('module', 'block', {
    ...
    set(el, caret, force) {
      ...
      // control focus
      if (
        this.opts.is('block.outline') &&
        this.opts.is('control') &&
        !this.instance.isFocusable() &&
        this.instance.getControl()
      ) {
    ...
    _setCaret(point) {
      ...
      if (point === 'column') {
        ...
        (this.instance.isInline() && !this.instance.isEditable())
      ) {
        this._setFocus();
  ...
  Redactor.add('module', 'context', {
    ...
    _getBlockSelection(e) {
      return this.dom(e.target).closest(
        '[data-rx-inline], [data-rx-type=noneditable], [data-rx-type=figcaption]'
      ).length !== 0
        ? false
        : true;
    },
  ...
  Redactor.add('module', 'control', {
    init() {
      ...
      this.parentTypes = {
        cell: [{ name: 'parent', title: '## table.select-table ##' }, 'cell-setting'],
        cellParent: [
    ...
    popup(e, button) {
      ...
      this.app.dropdown.create('control', {
        items: buttons,
  ...
  Redactor.add('module', 'embed', {
    ...
    edit(params, button) {
      let instance = this.app.block.get();
      let data = {
        embed: instance.getContent().replace(/(?<=\b\w+="[^"]*)&amp;(?=[^"]*")/g, '&'),
    ...
    _createInstance(data, code, current) {
      ...
      return this.app.create('block.embed', $figure, {
        caption: data.caption,
        responsive: data.responsive,
      });
    },
    _findScripts() {
      let scripts = this.app.editor
        .getLayout()
        .find('[data-rx-type=embed]')
        .find('script')
        .getAll();
  ...
  Redactor.add('module', 'event', {
    ...
    _onclick(e) {
      ...
      // triple click
      if (e.detail === 3 && !this._isToolClick(e)) {
        setTimeout(() => {
    ...
    _onkeyup(e) {
      ...
      if (e.which === 13) {
        setTimeout(() => {
          let element = this.app.create('element');
          let block = this.app.block.get().getBlock();
          let isVisible = element.isElementVisibleInScroll(block);
    ...
    _ondocmousedown(e) {
      if (!this.trigger) return;
      this.isPopupMouseUp =
        this.dom(e.target).closest('.rx-modal-' + this.uuid + ', .rx-dropdown-' + this.uuid)
          .length !== 0;
    ...
    _buildEventKeysObj(e) {
      ...
      let isAlphaKeys =
        !e.ctrlKey && !e.metaKey && ((key >= 48 && key <= 57) || (key >= 65 && key <= 90));
    ...
    _stopLinkEvents() {
      this.app.editor
        .getLayout()
        .on(
          'click.' + this.preventEventname + ' dblclick.' + this.preventEventname,
          this._stopLinkEvent.bind(this)
        );
    },
    _stopLinkEvent(e) {
    ...
    _isOutsideEditor(e) {
      ...
      return (
        $target.closest('.rx' + targets.join(this.uuid + ',.rx') + this.uuid).length === 0 &&
        $target.closest('.rx-editor-' + this.uuid).length === 0
      );
    },
    _isToolClick(e) {
      let $target = this.dom(e.target);
    ...
    _checkPanelKeys(e) {
      let key = e.which;
      if (
        this.app.panel.isOpen() &&
        (this._isEnter(e) || key === 40 || key === 39 || key === 38 || key === 37)
      ) {
        e.preventDefault();
        return true;
      }
    ...
    _setBlockByCoords(e) {
      ...
      closestIndex =
        heightIndex !== false ? heightIndex : distances.indexOf(Math.min.apply(Math, distances));
  ...
  Redactor.add('module', 'format', {
    ...
    popup(e, button) {
      ...
      this.app.dropdown.create('format', {
        items: buttons,
        extend: formatItems,
        keys: activeKeys,
    ...
    _format(instances) {
      ...
      instances.forEach(
        function (instance) {
          ...
          if (
            ...
            hasSameTag &&
            !this._isParams() &&
            (this.type === 'heading' || this.type === 'address')
          ) {
            ...
            if (
              (this.type === 'heading' || this.type === 'text') &&
              instance.isType(['list', 'todo'])
            ) {
      ...
      // convert formatted
      this.$editor.find('.rx-format-todo-to-listitem').each(
        function ($node) {
          this._convertFormattedTodoToList($node, elm);
        }.bind(this)
      );
      this.$editor.find('.rx-format-todo-to-text').each(
        function ($node) {
          this._convertFormattedTodoToText($node, elm);
        }.bind(this)
      );
      this.$editor.find('.rx-format-list-to-text').each(
        function ($node) {
          this._convertFormattedListToText($node, elm);
        }.bind(this)
      );
      this.$editor.find('.rx-format-listitem-to-todo').each(
        function ($node) {
          this._convertFormattedListToTodo($node, elm);
        }.bind(this)
      );
          ...
          let item = $node.dataget('instance'),
            ...
            block =
              this.type === 'text'
                ...
                : this.app.create('block.heading', {
                    level: this.tag.replace('h', ''),
                    content: content,
                  });
    ...
    _convertList(type, instance, elm) {
      ...
      if (type === 'text' || type === 'heading') {
        for (let i = 0; i < items.length; i++) {
          block =
            type === 'text'
              ? this.app.create('block.text', { content: items[i] })
              : this.app.create('block.heading', {
                  level: this.tag.replace('h', ''),
                  content: items[i],
                });
    ...
    _convertTodo(type, instance, elm) {
      ...
      if (type === 'text' || type === 'heading') {
        for (let i = 0; i < items.length; i++) {
          block =
            type === 'text'
              ? this.app.create('block.text', { content: items[i].content })
              : this.app.create('block.heading', {
                  level: this.tag.replace('h', ''),
    ...
    _replaceToHeading(instance) {
      let content = instance.getContent();
      let block = this.app.create('block.heading', {
        level: this.tag.replace('h', ''),
        content: content,
      });
      instance.getBlock().after(block.getBlock());
    ...
    _replaceListTodoToText(mode, type, instance) {
      let content = instance.getContent(true);
      let block =
        mode === 'text'
          ? this.app.create('block.text', { content: content })
          : this.app.create('block.heading', {
      ...
      let $block = block.getBlock();
      let $items = instance.getBlock().find('li');

      instance.getBlock().after($block);
      $items.each(($node) => {
        let itemInstance = $node.dataget('instance');
        let itemBlock =
          mode === 'text'
            ? this.app.create('block.text', { content: itemInstance.getContent(true) })
            : this.app.create('block.heading', {
                level: this.tag.replace('h', ''),
                content: itemInstance.getContent(true),
              });
  ...
  Redactor.add('module', 'hotkeys', {
    ...
    popup(e, button) {
      ...
      this._buildPopupItems(items, z, this.opts.get('hotkeys'), meta);
    ...
    _buildPopupItems(items, z, hotkeys, meta, type) {
      for (let [key, item] of Object.entries(hotkeys)) {
        let $item = this.dom('<div>'),
          ...
          $title = this.dom('<span>')
            .addClass('rx-dropdown-item-title')
        ...
        for (let i = 0; i < arr.length; i++) {
          if (arr[i] === 'Cmd' || arr[i] === 'Ctrl') {
            ...
            arr[i] =
              '<span>' + (arr[i].search(/&/) === -1 ? arr[i].toUpperCase() : arr[i]) + '</span>';
          }
        }
        $kbd.html(arr.join('+'));
  ...
  Redactor.add('module', 'image', {
    ...
    _createImageFromResponseItem(item) {
      let $image = this.dom('<img>')
        .attr('src', item.url)
        .one('load', this._checkImageLoad.bind(this));
    ...
    _createOrSection($modal) {
      if (
        this.opts.is('image.url') &&
        (this.opts.is('image.upload') || this.opts.is('image.select'))
      ) {
    ...
    _parseList(data, callback) {
      for (let [key, item] of Object.entries(data)) {
        ...
        $img.on(
          'click.rx-modal-event-' + this.uuid,
          function (e) {
            let $target = this.dom(e.target);
  ...
  Redactor.add('module', 'inline', {
    ...
    popup(e, button) {
      ...
      this.app.dropdown.create('inline', {
        items: buttons,
        extend: inlineItems,
        keys: activeKeys,
        type: 'inlinebar',
    ...
    _formatCollapsed() {
      ...
      // 3) inline isn't empty
        ...
        if (hasSameTag) {
          ...
          if (this._isParams()) {
            // 3.1.1) params
            if (this._hasAllSameParams(inlineTop)) {
              // all the same
    ...
    _revertInlineBlocks($blocks) {
      $blocks
        .find('.rx-inlineblock-editable')
        .removeClass('rx-inlineblock-editable')
        .attr('contenteditable', true);
    },
    _revertTag(tag, $blocks) {
      let elm = this.app.create('element');
      $blocks.find('span.rx-convertable-' + tag).each(
        function (node) {
    ...
    _revertToInlines($blocks) {
      ...
      // span convertable
      $blocks.find('.rx-inline-convertable').each(
        function ($node) {
      ...
      // span unconvertable
      $blocks.find('span.rx-inline-unconvertable').each(
        function ($node) {
    ...
    _isFullySelected(node, selected) {
      ...
      return (
        selected === text ||
        selected.search(new RegExp('^' + utils.escapeRegExp(text) + '$')) !== -1
      );
    },
  });
  /*jshint esversion: 6 */
    ...
    observe(obj, name, toolbar) {
      ...
      if (toolbar === 'context' && name === 'link' && selection.is()) {
        ...
        if ($links.length === 1) {
          ...
          this.app.context.addLine(
            '<a href="' + linkText + '">' + this._truncateLinkText(linkText) + '</a>'
          );
  ...
  Redactor.add('module', 'observer', {
    ...
    _getKeys() {
      let instance = this.app.block.get(),
        ...
        selection = this.app.create('selection'),
        inlines = selection.is()
          ? selection.getNodes({ type: 'inline', selected: 'inside', link: true, buttons: true })
          : [],
  ...
  Redactor.add('module', 'state', {
    ...
    add(e) {
      if (
        (e && (e.ctrlKey || e.metaKey || this._isUndo(e) || this._isRedo(e))) ||
        !this.app.observer.trigger
      ) {
        return;
      }
  ...
  Redactor.add('module', 'table', {
    ...
    _addColumn(name) {
      ...
      let instance = this.app.block.get();
      let $block = instance.isType('cell')
        ? instance.getBlock()
        : instance.getClosest('cell').getBlock();
      let $table = $block.closest('table');
      let $row = $block.closest('tr');
  ...
  Redactor.add('module', 'scroll', {
    ...
    isTarget() {
      return this.opts.is('scrollTarget');
    },
    resetTarget() {
      if (this.reserved) {
  ...
  Redactor.add('module', 'reorder', {
    build(control, button, $control, instance) {
      ...
      this.$button.on(
        'mousedown.rx-reorder-button touchstart.rx-reorder-button',
        this._press.bind(this)
      );
    },
    ...
    _press(e) {
      ...
      this.$button.on(
        'touchend.rx-reorder-button mouseup.rx-reorder-button',
        this._checkTap.bind(this)
      );
    },
    _checkTap(e) {
    ...
    _bindWindowEvents() {
      this.app
        .getWin()
        .on('mouseup.rx-reorder-button touchend.rx-reorder-button', this._release.bind(this));
      this.app
        .getWin()
        .on('mousemove.rx-reorder-button touchmove.rx-reorder-button', this._move.bind(this));
    },
    _release(e) {
  ...
  Redactor.add('module', 'dropdown', {
    ...
    _startEvents() {
      this.app.scroll
        .getTarget()
        .on('resize.rx-dropdown scroll.rx-dropdown', this.updatePosition.bind(this));
    ...
    _select(type) {
      ...
      this.$dropdown.scrollTop(
        scrollTop > itemsScrollTop + itemsHeight
          ? scrollTop - itemsHeight
          : itemTop - itemHeight < itemsScrollTop
            ? itemTop - itemHeight
            : itemsScrollTop
      );
    },
  ...
  Redactor.add('module', 'autosave', {
    ...
    _send() {
      ...
      this.ajax.request(method, {
        ...
        data: data,
        before: function (xhr) {
          let event = this.app.broadcast('autosave.before.send', {
            xhr: xhr,
            name: name,
            data: data,
          });
  ...
  Redactor.add('module', 'input', {
    ...
    handleArrow(e, key, event) {
      ...
      if (this.app.editor.isSelectAll()) {
        e.preventDefault();
        let target = event.is('down-right')
          ? this.app.blocks.get({ last: true })
          : this.app.blocks.get({ first: true });
      ...
      // multiple selection
        ...
        e.preventDefault();
        instance = event.is('down-right')
          ? this.app.blocks.get({ last: true, selected: true })
          : this.app.blocks.get({ first: true, selected: true });
      ...
      if (
        instance.isEditable() &&
        event.is(['left', 'right']) &&
        this._trimInvisibleChar(e, event.is('left') ? 'left' : 'right')
      ) {
        return;
      }
    ...
    _isAllSelected(event) {
      return (
        ...
        event.is(['enter', 'delete', 'backspace', 'alpha', 'space'])
      );
    },
  ...
  Redactor.add('module', 'command', {
    ...
    _insertFromPanel(e, $el) {
      ...
      if (n >= 0) {
        currentText =
          currentText.substring(0, n) + replacement + currentText.substring(n + what.length);
      }
  ...
  Redactor.add('module', 'ui', {
    ...
    close(e, ...contexts) {
      ...
      const validContexts = this._handleContexts(contexts, [
        'dropdown',
        'modal',
        'control',
        'context',
    ...
    updateTheme(theme) {
      ...
      popups.forEach((popup) =>
        this.app.$body
          .find(`.rx-${popup}-` + this.uuid)
          .each(($node) => $node.attr('rx-data-theme', theme))
      );
    },
    ...
    _buildObject(type, name, extend) {
      ...
      btnObj = btnObj
        ? Redactor.extend(true, {}, btnObj, extend)
        : Redactor.extend(true, {}, extend);
    ...
    _isHiddenButton(type, name) {
      ...
      if (
        name === 'trash' &&
        instance &&
        instance.isType('noneditable') &&
        !this.opts.is('noneditable.remove')
      )
        return true;
      if (name === 'trash' && instance && instance.isNondeletable()) return true;
      if ((name === 'trash' || name === 'duplicate') && instance && instance.isType('column'))
        return true;
  ...
  class UIBuilder {
    ...
    build() {
      this.$toolbar = new Dom('<div>').addClass(
        'rx-' + this.type + ' rx-' + this.type + '-' + this.uuid
      );
  ...
  Redactor.add('block', 'dlist', {
    ...
    getItems() {
      ...
      this.$block.find('dt').each(($node) => {
        ...
        const descHtml =
          $next && $next.tagName('dd') ? this.unparseInlineBlocks($next.clone()).html() : '';
  ...
  Redactor.add('block', 'figcaption', {
    ...
    handleArrow(e, key, event) {
      if (
        (event.is('up-left') && this.isCaretStart()) ||
        (event.is('down-right') && this.isCaretEnd())
      ) {
        e.preventDefault();
  ...
  Redactor.add('block', 'layout', {
    ...
    getColumns() {
      return this.$block
        .find('[data-rx-type=column]')
        .map((node) => this.dom(node).dataget('instance'));
    },
  ...
  Redactor.add('block', 'list', {
    ...
    getItems() {
      return this.$block
        .children()
        .getAll()
        .map((node) => this.dom(node).dataget('instance').getContent());
    },
  ...
  Redactor.add('block', 'quote', {
    ...
    _parseCaption() {
      let $caption = this.isFigure()
        ? this.$block.find('figcaption')
        : this.$block.find('p').last();
  ...
  Redactor.add('block', 'cell', {
    ...
    setNowrap(value) {
      ...
      this._applyToEachCell(($cell) =>
        value ? $cell.addClass(classes) : $cell.removeClass(classes)
      );
    },
    setEmpty() {
      let emptyInstance = this.app.block.create();
  ...
  Redactor.add('block', 'todoitem', {
    ...
    _parse() {
      ...
      this.$content =
        $content.length !== 0 ? $content : this.dom(this.opts.get('todo.templateContent'));
    ...
    _updateItemState(html, $input, $content, itemTemplate, itemDoneTemplate) {
      ...
      if (html.startsWith(itemDoneTemplate)) {
        ...
        html = html.replace(
          html.startsWith(itemTemplateTrim) ? itemTemplateTrim : itemTemplate,
          ''
        );
