...
Redactor.add('plugin', 'ai', {
  ...
  dropdowns: {
    items: {
      ...
      shorten: {
        ...
        command: 'ai.set',
        params: { prompt: 'Make it shorter' },
      },
      detailed: {
        title: 'Make it more detailed',
        command: 'ai.set',
        params: { prompt: 'Make it more detailed' },
      },
      complete: {
        title: 'Complete sentence',
        command: 'ai.set',
        params: { prompt: 'Complete sentence' },
      },
  ...
  defaults: {
    ...
    style: ['3d model', 'Digital art', 'Isometric', 'Line art', 'Photorealistic', 'Pixel art'],
    translate: [
      'Arabic',
      'Chinese',
      'English',
  ...
  popupTranslate(e, button) {
    ...
    for (let i = 0; i < items.length; i++) {
      buttons[i] = {
        title: items[i],
        command: 'ai.set',
        params: { prompt: translateto + ' ' + items[i] },
      };
    }
  ...
  sendPrompt(e) {
    ...
    request =
      this.promptType === 'image'
        ? { model: apimodel, n: 1, size: size, prompt: message }
        : request;
  ...
  _buildPrompt(params) {
    ...
    if (instance || isMultiple) {
      ...
      let $parent = instance
        .getBlock()
        .closest('[data-rx-type=' + types.join('],[data-rx-type=') + ']');
  ...
  _createTone($select) {
    let items =
      this.promptType === 'image'
        ? this.opts.get('ai.style') || this.defaults.style
        : this.opts.get('ai.tone') || this.defaults.tone;
    let name =
      this.promptType === 'image'
        ? this.lang.get('ai.image-style')
        : this.lang.get('ai.change-tone');
  ...
  _createPromptButtons($buttons) {
    this.$generate = this._createPromptButton(this.lang.get('ai.send'))
      .addClass('rx-form-button-primary')
      .on('click.rx-ai', this.sendPrompt.bind(this));
    this.$stop = this._createPromptButton(this.lang.get('ai.stop'))
      .addClass('rx-form-button-primary')
      .on('click.rx-ai', this.stopPrompt.bind(this))
      .hide();
    this.$discard = this._createPromptButton(this.lang.get('ai.discard'))
      .addClass('rx-form-button-danger')
      .on('click.rx-ai', this.closePrompt.bind(this));
    this.$insert = this._createPromptButton(this.lang.get('ai.insert'))
      .on('click.rx-ai', this.insertPrompt.bind(this))
      .hide();
